...
else
{
	...
	$sql="SELECT Max(ProID) +1  AS ProductCount FROM product_info";
    $Data= getJSONFromDB($sql);
    $Data = json_decode($Data, true);	
	
	$fname = $_FILES['fileToUpload']['name'];
	$ExtName = explode(".", $fname);
	$ProductName = "uploads/image".$Data[0]["ProductCount"].".".$ExtName[1];
	
	
	move_uploaded_file($_FILES["fileToUpload"]["tmp_name"], $ProductName);
	
	$sql="INSERT INTO product_info (ProID, ProName,ProDescription,ProPrice,ProPicture,ProCondition,ProCategory,ProLocation,Username)
